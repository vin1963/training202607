...
| 前端技術 | Angular 17 (Standalone Components) 或 React 18 + React Router v6 |
...
5. [前端 React + Routing 實作](#5-前端-react--routing-實作)
6. [前端 Vite + React + Routing + JavaScript 實作](#6-前端-vite--react--routing--javascript-實作)
7. [API 規格總表](#7-api-規格總表)
8. [SQL 初始化腳本](#8-sql-初始化腳本)
...
```
┌─────────────────────────────────────────────────────────────┐
│               前端（Angular 或 React，port 4200）             │
│                                                             │
│  Angular:          React:                                   │
│  ┌──────────────┐  ┌────────────────────────────────────┐   │
│  │ Dashboard    │  │  Dashboard.tsx                     │   │
│  │ ProductList  │  │  ProductList.tsx / ProductDetail   │   │
│  │ LowStockAlert│  │  LowStockAlert.tsx                 │   │
│  │  (Standalone)│  │  (Functional + Hooks)              │   │
│  └──────┬───────┘  └────────────────┬───────────────────┘   │
│         └───────────────────────────┤                       │
│         HttpClient / Axios          │                       │
└─────────────────────────────────────┼───────────────────────┘
...
## 5. 前端 React + Routing 實作

> 本章節使用 **React 18 + React Router v6 + Bootstrap 5** 實作與 Angular 版功能完全相同的前端。
...
├── App.tsx                          ← 根元件（含導覽列 + Routes）
├── models/
│   ├── product.ts                   ← Product / ApiResponse 介面
│   └── stock-summary.ts             ← StockSummary / StockThreshold 介面
├── services/
│   └── productService.ts            ← Axios 封裝（對應 Angular ProductService）
├── pages/
│   ├── Dashboard.tsx                ← 儀表板頁面
│   ├── ProductList.tsx              ← 產品列表頁面
│   └── LowStockAlert.tsx            ← 低庫存警示頁面
└── components/
    └── Navbar.tsx                   ← 導覽列元件
```
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          <table className="table table-borderless mb-0">
            <tbody>
              <tr><th width="150">產品名稱</th><td>{product.productName}</td></tr>
...
### 5.10 Angular vs React 路由對照表

| 功能 | Angular (app.routes.ts) | React (App.tsx) |
|------|------------------------|-----------------|
| 預設導向 | `{ path: '', redirectTo: 'dashboard' }` | `<Navigate to="/dashboard" replace />` |
| 儀表板 | `{ path: 'dashboard', component: DashboardComponent }` | `<Route path="/dashboard" element={<Dashboard />} />` |
| 產品列表 | `{ path: 'products', component: ProductListComponent }` | `<Route path="/products" element={<ProductList />} />` |
| 產品明細 | `{ path: 'products/:code', component: ProductDetailComponent }` | `<Route path="/products/:id" element={<ProductDetail />} />` |
| 低庫存警示 | `{ path: 'low-stock', component: LowStockAlertComponent }` | `<Route path="/low-stock" element={<LowStockAlert />} />` |
| 路由參數 | `ActivatedRoute.params` | `useParams()` Hook |
| 導覽列 active | `routerLinkActive="active"` | `<NavLink className={({isActive}) => ...}>` |
| 程式化導覽 | `this.router.navigate(['/products'])` | `useNavigate()` Hook |

---

### 5.11 Angular vs React 狀態管理對照表

| 概念 | Angular | React |
|------|---------|-------|
| 元件狀態 | `this.xxx = value` | `useState()` Hook |
| 生命週期 | `ngOnInit()` | `useEffect(() => {...}, [])` |
| 模板雙向綁定 | `[(ngModel)]="xxx"` | `value={xxx}` + `onChange` |
| 條件渲染 | `*ngIf="xxx"` | `{xxx && <Component />}` |
| 迭代渲染 | `*ngFor="let item of list"` | `{list.map(item => ...)}` |
| 服務注入 | `constructor(private svc: Service)` | 直接 `import { func } from '...'` |
| HTTP 服務 | `HttpClient` + `Observable` | `axios` + `async/await` 或 `fetch` |
| 子元件溝通 | `@Input()` / `@Output()` | Props / Callback |

---

## 6. 前端 Vite + React + Routing + JavaScript 實作

> 本章節使用 **Vite + React 18 + React Router v6 + Bootstrap 5 + 純 JavaScript（無 TypeScript）** 實作。
...
└── src/
    ...
    ├── App.jsx                    ← 根元件（含導覽列 + Routes）
    ├── api/
    │   └── productService.js      ← axios 封裝
    ├── pages/
    │   ├── Dashboard.jsx          ← 儀表板頁面
    │   ├── ProductList.jsx        ← 產品列表頁面
    │   ├── ProductDetail.jsx      ← 產品明細頁面
    │   └── LowStockAlert.jsx      ← 低庫存警示頁面
    └── components/
        └── Navbar.jsx             ← 導覽列元件
```
...
> ⚠️ **重點說明**：`proxy` 設定可避免 CORS 問題，前端所有 `/api` 請求會自動轉發到後端 `localhost:8080`，因此 `productService.js` 中的 `API_URL` 可直接設為空字串。
...
export async function getProductById(productCode) {
  return unwrap(api.get(`/api/products/${productCode}`));
}

export async function createProduct(product) {
  return unwrap(api.post('/api/products', product));
}

export async function updateProduct(productCode, product) {
  return unwrap(api.put(`/api/products/${productCode}`, product));
}
...
export async function getThreshold(productCode) {
  return unwrap(api.get(`/api/thresholds/${productCode}`));
}

export async function saveThreshold(productCode, minQuantity, reorderQuantity) {
  return unwrap(
    api.put(`/api/thresholds/${productCode}`, { minQuantity, reorderQuantity })
  );
}
```
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-dark bg-dark">
      <div className="container">
        ...
        <ul className="navbar-nav">
          <li className="nav-item">
            <NavLink className={linkClass} to="/dashboard">儀表板</NavLink>
          </li>
          <li className="nav-item">
            <NavLink className={linkClass} to="/products">產品管理</NavLink>
          </li>
          <li className="nav-item">
            <NavLink className={linkClass} to="/low-stock">低庫存警示</NavLink>
          </li>
        </ul>
      </div>
    </nav>
...
function Dashboard() {
  ...
  function getStatusLabel(minStock) {
    if (minStock < 100) return '緊急補貨';
    if (minStock < 500) return '低庫存';
    return '正常';
  }

  function getStatusClass(minStock) {
    if (minStock < 100) return 'danger';
    if (minStock < 500) return 'warning';
    return 'success';
  }
          ...
          <tbody>
            {lowStockProducts.map((p) => (
              <tr key={p.productCode}>
                ...
                <td className="text-center">
                  <Link to={`/products/${p.productCode}`}
...
function ProductList() {
  ...
  function calcMargin(buyPrice, msrp) {
    if (msrp === 0) return '0%';
    return (((msrp - buyPrice) / msrp) * 100).toFixed(1) + '%';
  }
      ...
      <div className="row g-2 mb-3">
        ...
        <div className="col-md-1">
          <button className="btn btn-primary w-100"
                  onClick={loadProducts}>搜尋</button>
        </div>
        <div className="col-md-1">
          <button className="btn btn-secondary w-100" onClick={() => {
            setSelectedLine('');
...
function LowStockAlert() {
  ...
  function handleSave(productCode) {
    saveThreshold(productCode, editMinQty, editReorderQty).then((t) => {
      setThresholdMap((prev) => ({ ...prev, [productCode]: t }));
      setEditingCode('');
    });
            ...
            lowStockProducts.map((p) => {
              ...
              return (
                <tr key={p.productCode}>
                  ...
                  <td>{p.productVendor}</td>
                  <td className="text-end">
                    <span className={p.quantityInStock < 100
                      ? 'text-danger fw-bold'
                    ...
                    {isEditing ? (
                      <>
                        <button className="btn btn-success btn-sm"
                                onClick={() => handleSave(p.productCode)}>
                          儲存
                        </button>
...
| 項目 | Angular 17 | React + TypeScript | Vite React + JS |
|------|-----------|-------------------|-----------------|
| 建構工具 | Angular CLI | Create React App | Vite |
| 語言 | TypeScript | TypeScript | **JavaScript** |
| 路由 | `Routes` (app.routes.ts) | `Routes` (App.tsx) | **Routes (App.jsx)** |
| 路由參數 | `ActivatedRoute.params` | `useParams()` | **useParams()** |
| 導覽列 active | `routerLinkActive` | `<NavLink>` | **NavLink** |
| HTTP 服務 | `HttpClient` (Observable) | `axios` (async/await) | **axios (async/await)** |
| 狀態管理 | 元件屬性 + constructor | `useState()` Hook | **useState() Hook** |
| 生命週期 | `ngOnInit()` | `useEffect()` | **useEffect()** |
| 模板語法 | `*ngIf` / `*ngFor` | `{}` JSX | **{} JSX** |
| 表單綁定 | `[(ngModel)]` | `value` + `onChange` | **value + onChange** |
| 預設端口 | 4200 | 3000 | **5173** |
| CORS 處理 | Angular proxy 或後端設定 | 後端允許 3000 | **Vite proxy 轉發** |
...
> ⚠️ **注意**：使用 Vite proxy 後，後端 `CorsConfig.java` 不需額外設定前端端口（因請求由 Vite 伺服器轉發，對後端而言是同源請求）。
...
#### React 版（Create React App）
```bash
# 1. 建立 React 專案
npx create-react-app inventory-react --typescript
cd inventory-react
npm install react-router-dom axios bootstrap
